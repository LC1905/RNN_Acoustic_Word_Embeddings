...
transcript_path = "../full_swbd_trans/swb_ms98_transcriptions/"
...
def partition_file(dt):
	with open("../full_utts_swbd/mfccs_file_1.pkl", "rb") as f:
		print("loading data...")
		data = pickle.load(f)
...
def input1(train_path):
	train = np.load(train_path + "train.npz")
	keys = list(train.keys())
	values = [train[key] for key in keys]
	np.save(train_path + "input1.npy", values)
...
def input2(train_path):
	...
	np.save(train_path + "input2.npy", values)
