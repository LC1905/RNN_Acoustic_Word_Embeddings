...
class SimpleLSTM(object):
    def __init__(self, is_training, config):
        ...
        r2l_outputs_view1 = tf.gather(tf.reshape(tf.concat(r2l_outputs_layer2_view1, 1), [-1, hidden_size]),
                                      tf.range(tf.shape(input_x1)[0]) * tf.shape(input_x1)[1] + input_x1_lengths - 1)
        self._final_state = x1 = self.normalization(tf.concat([l2r_outputs_view1, r2l_outputs_view1], 1, 'concat_view1_x1'))
        ...
        if 2 in obj or 3 in obj:
            ...
            # View 1 Layer 2 x2
            input_x2_layer2 = tf.concat([l2r_outputs_layer1_view1, r2l_outputs_layer1_view1],2, 'concat_layer1_view1_x2')
            ...
            r2l_outputs_view1 = tf.gather(tf.reshape(tf.concat(r2l_outputs_layer2_view1, 1), [-1, hidden_size]),
                                          tf.range(tf.shape(input_x2)[0]) * tf.shape(input_x2)[
                                              1] + input_x2_lengths - 1)
            x2 = self.normalization(tf.concat([l2r_outputs_view1, r2l_outputs_view1], 1, 'concat_view1_x2'))
        ...
        # View 2 Layer 2 c1
        input_c1_layer2 = tf.concat([l2r_outputs_layer1_view2, r2l_outputs_layer1_view2], 2, 'concat_layer1_view2_c1')
...
def main():
    ...
    print "loading data..."
    input_x1_set, input_c1_set, input_x1_lengths_set, input_c1_lengths_set= dp.GetInputData('../word_data/', conf)
    dev = np.load('../word_data/dev.npz')
    train = np.load('../word_data/train.npz')
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
        ...
        for i in range(last_epoch + 1, last_epoch + conf.epoch + 1):
            ...
            #print "Dev AP: " + str(dev_ap) + "\n"
            #print "Train AP: " + str(train_ap) + "\n"
